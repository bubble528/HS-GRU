...
class HRED():
    ...
    def __init__(self, vocab_size=50004, embedding_dim=300, query_dim=1000, session_dim=1500,
                 ...
                 d_a_size=350, r_size=5, p_coef=1.0, step=1):#
    ...
    def non_padding_accuracy(self, logits, labels):
        ...
        # labels = tf.Print(labels, [labels], summarize=1500)
        logits_select = tf.argmax(self.softmax(logits), 2)#, axis=2
        # logits_select = tf.Print(logits_select, [logits_select], summarize=1500)

        # Matching non-padding tokens
        correct_prediction = tf.reduce_sum(
            tf.cast(
                tf.logical_and(
                    tf.equal(logits_select, labels),
        ...
        tf.summary.scalar("non-padding-acc", non_padding_acc)
    ...
    def non_symbol_accuracy(self, logits, labels):
        ...
        correct_prediction = tf.reduce_sum(
            tf.cast(
                tf.logical_and(
                    tf.equal(tf.argmax(self.softmax(logits), 2), labels),
                    tf.logical_and(
                        tf.not_equal(tf.constant(self.eos_symbol, tf.int64), labels),
                        tf.logical_and(
        ...
        tf.summary.scalar("non-symbol-acc", non_symbol_acc)
